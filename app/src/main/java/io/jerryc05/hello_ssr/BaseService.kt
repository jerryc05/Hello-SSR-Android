	...
	var binder: IShadowsocksService.Stub = object : IShadowsocksService.Stub()
	{
		...
		override fun registerCallback(cb: IShadowsocksServiceCallback?)
		{
			if (cb != null && callbacks.register(cb))
			{
				...
				{
					VayLog.e(BaseService.Companion.TAG, "registerCallback", e)
			...
			else
			{
				...
				else
				{
					when (currentState)
					{
						...
						else -> VayLog.w(BaseService.Companion.TAG, "Illegal state when invoking use: $currentState")
					}
				}
			}
		}
...
//			}
	...
	fun startRunner(profile: Profile)
	{
		...
		ShadowsocksApplication.Companion.app.track(BaseService.Companion.TAG, "start")

		changeState(Constants.State.CONNECTING)

		if (profile.isMethodUnsafe)
		{
			BaseService.Companion.handler.post { ToastUtils.showLong(R.string.method_unsafe) }
		}
		...
		catch (e: BaseService.NameNotResolvedException)
		{
			stopRunner(true, getString(R.string.invalid_server))
		}
		catch (e: BaseService.NullConnectionException)
		{
	...
	{
		BaseService.Companion.handler.post {
			if (callbacksCount > 0)
			{
