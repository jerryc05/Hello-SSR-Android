...
abstract class BaseVpnService : VpnService()
{
	...
	var binder: IShadowsocksService.Stub = object : IShadowsocksService.Stub()
	{
		...
		override fun registerCallback(cb: IShadowsocksServiceCallback?)
		{
			if (cb != null && callbacks.register(cb))
			{
				...
				{
					VayLog.e(BaseVpnService.Companion.TAG, "registerCallback", e)
		...
		override fun use(profileId: Int)
		{
			...
			{
				val profile = ShadowsocksApplication.Companion.app.profileManager.getProfile(profileId)
				if (profile == null)
				{
					stopRunner(true)
				}
				else
				{
					when (currentState)
					{
						...
						else -> VayLog.w(BaseVpnService.Companion.TAG, "Illegal state when invoking use: $currentState")
					}
				}
			}
		}
	...
	val blackList: String
		get()
		{
			...
			try
			{
				val container = ShadowsocksApplication.Companion.app.containerHolder.container
	...
	open fun connect()
	{
		...
		if (Constants.DefaultHostName == profile!!.host)
		{
			...
			{
				VayLog.e(BaseVpnService.Companion.TAG, "connect", e)
				ShadowsocksApplication.Companion.app.track(e)
	...
	open fun startRunner(profile: Profile)
	{
		...
		ShadowsocksApplication.Companion.app.track(BaseVpnService.Companion.TAG, "start")
		...
		catch (e: BaseVpnService.NameNotResolvedException)
		{
			stopRunner(true, getString(R.string.invalid_server))
		}
		catch (e: BaseVpnService.NullConnectionException)
		{
		...
		catch (exc: Throwable)
		{
			...
			exc.printStackTrace()
			ShadowsocksApplication.Companion.app.track(exc)
		}
	}
	...
	private fun updateTrafficTotal(tx: Long, rx: Long)
	{
		...
		if (profile != null)
		{
			val p = ShadowsocksApplication.Companion.app.profileManager.getProfile(profile.id)
			if (p != null)
			{
				// default profile may have host, etc. modified
				p.tx = p.tx + tx
				p.rx = p.rx + rx
				ShadowsocksApplication.Companion.app.profileManager.updateProfile(p)
			}
		}
	}
	...
	{
		super.onCreate()
		ShadowsocksApplication.Companion.app.refreshContainerHolder()
		ShadowsocksApplication.Companion.app.updateAssets()
	}
	...
	companion object
	{
		val protectPath = "${ShadowsocksApplication.Companion.app.applicationInfo.dataDir}/protect_path"
