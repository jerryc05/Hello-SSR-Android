		...
		guardThread = Thread(object : Runnable
							 {
								 override fun run()
								 {
									 try
									 {
										 ...
										 while (!isDestroyed)
										 {
											 VayLog.i(GuardedProcess.Companion.TAG, "start process: $cmd")
											 ...
											 StreamLogger(inputStream, GuardedProcess.Companion.TAG).start()
											 ...
											 synchronized(this) {
												 ...
												 else
												 {
													 ...
													 {
														 Log.w(GuardedProcess.Companion.TAG, "process exit too fast, stop guard: $cmd")
														 isDestroyed = true
													 }
												 }
											 }
									 ...
									 {
										 VayLog.i(GuardedProcess.Companion.TAG, "thread interrupt, destroy process: $cmd")
										 if (process != null)
										 {
											 process!!.destroy()
										 }
