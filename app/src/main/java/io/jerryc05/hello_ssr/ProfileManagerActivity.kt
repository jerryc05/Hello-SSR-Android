...
import io.jerryc05.hello_ssr.network.ping.*
...
import io.jerryc05.hello_ssr.widget.*
...
class ProfileManagerActivity : AppCompatActivity(), View.OnClickListener, Toolbar.OnMenuItemClickListener, NfcAdapter.CreateNdefMessageCallback, ProfileManager.ProfileAddedListener, SSRSubManager.SSRSubAddedListener
{
	...
	private var selectedItem: ProfileManagerActivity.ProfileViewHolder? = null
	private val handler = Handler()

	private lateinit var profilesAdapter: ProfileManagerActivity.ProfilesAdapter
	private lateinit var ssrSubAdapter: ProfileManagerActivity.SSRSubAdapter
	...
	private val mProgressHandler = object : Handler(Looper.getMainLooper())
	{
		override fun handleMessage(msg: Message)
		{
			when (msg.what)
			{
				ProfileManagerActivity.Companion.MSG_FULL_TEST_FINISH ->
				{
	...
	private val currentProfilePosition: Int
		get()
		{
			...
			for (i in profiles.indices)
			{
				...
				if (profile.id == ShadowsocksApplication.Companion.app.profileId())
				{
					position = i
				}
			}
	...
	override fun onCreate(savedInstanceState: Bundle?)
	{
		...
		ShadowsocksApplication.Companion.app.profileManager.addProfileAddedListener(this)
			...
			ItemTouchHelper(object : ItemTouchHelper.SimpleCallback(ItemTouchHelper.UP or ItemTouchHelper.DOWN, ItemTouchHelper.START or ItemTouchHelper.END)
							{
								override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int)
								{
									...
									undoManager.remove(index, (viewHolder as ProfileManagerActivity.ProfileViewHolder).item)
								}
	...
	private fun initGroupSpinner()
	{
		...
		val groupNames = ShadowsocksApplication.Companion.app.profileManager.groupNames.toMutableList()
	...
	{
		if (ShadowsocksApplication.Companion.app.settings.getBoolean(Constants.Key.profileTip, true))
		{
			ShadowsocksApplication.Companion.app.editor.putBoolean(Constants.Key.profileTip, false)
				.apply()
	...
	private fun qrCodeScan()
	{
		try
		{
			...
			startActivityForResult(intent, ProfileManagerActivity.Companion.requestQrCode)
		}
		catch (e: Throwable)
		{
	...
	override fun onClick(v: View)
	{
		when (v.id)
		{
			...
			{
				menu.toggle(true)
				val profile = ShadowsocksApplication.Companion.app.profileManager.createProfile()
				ShadowsocksApplication.Companion.app.profileManager.updateProfile(profile)
				ShadowsocksApplication.Companion.app.switchProfile(profile.id)
				finish()
			}
			R.id.fab_qrcode_add ->
			{
	...
	private fun showUrlAddProfileDialog(profiles: List<Profile>)
	{
		val dialog = AlertDialog.Builder(this, R.style.Theme_Material_Dialog_Alert)
			...
			.setPositiveButton(android.R.string.yes) { _, _ ->
				...
				{
					ShadowsocksApplication.Companion.app.profileManager.createProfile(item)
				}
			}
			.setNeutralButton(R.string.dr) { _, _ ->
				for (item in profiles)
				{
					ShadowsocksApplication.Companion.app.profileManager.createProfileDr(item)
				}
			}
	...
	fun ssrSubDialog()
	{
		...
		// adding listener
		ShadowsocksApplication.Companion.app.ssrSubManager.addSSRSubAddedListener(this)
		...
		AlertDialog.Builder(this)
			...
			.setOnCancelListener {
				// remove listener
				ShadowsocksApplication.Companion.app.ssrSubManager.removeSSRSubAddedListener(this@ProfileManagerActivity)
			}
			.setView(view)
			.create()
			.show()
	...
	private fun delSubDialog(viewHolder: RecyclerView.ViewHolder)
	{
		...
		AlertDialog.Builder(this@ProfileManagerActivity)
			...
			.setPositiveButton(R.string.ssrsub_remove_tip_direct) { dialog, which ->
				...
				ShadowsocksApplication.Companion.app.ssrSubManager.delSSRSub((viewHolder as ProfileManagerActivity.SSRSubViewHolder).item.id)
			}
			...
			.setNeutralButton(R.string.ssrsub_remove_tip_delete) { _, _ ->
				val group = (viewHolder as ProfileManagerActivity.SSRSubViewHolder).item.url_group
				val deleteProfiles = ShadowsocksApplication.Companion.app.profileManager.getAllProfilesByGroup(group)

				for (profile in deleteProfiles)
				{
					if (profile.id != ShadowsocksApplication.Companion.app.profileId())
					{
						ShadowsocksApplication.Companion.app.profileManager.delProfile(profile.id)
					}
				}

				val index1 = viewHolder.getAdapterPosition()
				ssrSubAdapter.remove(index1)
				ShadowsocksApplication.Companion.app.ssrSubManager.delSSRSub(viewHolder.item.id)
	...
	private fun confirmWithUpdateSub()
	{
		...
		// start update sub
		val subs = ShadowsocksApplication.Companion.app.ssrSubManager.allSSRSubs
		...
		if (subUrl.isNotEmpty())
		{
			...
			RequestHelper.instance()!![subUrl, object : RequestCallback()
			{
				override fun onSuccess(code: Int, response: String)
				{
					...
					ShadowsocksApplication.Companion.app.ssrSubManager.createSSRSub(ssrSub)
				}
	...
	private fun handleShareIntent(intent: Intent)
	{
		...
		val dialog = AlertDialog.Builder(this, R.style.Theme_Material_Dialog_Alert)
			...
			.setPositiveButton(android.R.string.yes) { _, _ ->
				...
				{
					ShadowsocksApplication.Companion.app.profileManager.createProfile(profile)
				}
			}
			.setNeutralButton(R.string.dr) { _, _ ->
				for (profile in profiles)
				{
					ShadowsocksApplication.Companion.app.profileManager.createProfileDr(profile)
				}
			}
			.setNegativeButton(android.R.string.no) { _, _ -> finish() }
			.setMessage(Utils.makeString(profiles, "\n"))
	...
	override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?)
	{
		...
		if (requestCode == ProfileManagerActivity.Companion.requestQrCode)
		{
			when (resultCode)
			{
				Activity.RESULT_OK ->
	...
	override fun onDestroy()
	{
		...
		undoManager.flush()
		ShadowsocksApplication.Companion.app.profileManager.removeProfileAddedListener(this)
		super.onDestroy()
	}
	...
	override fun onMenuItemClick(item: MenuItem): Boolean
	{
		when (item.itemId)
		{
			R.id.action_export ->
			{
				val allProfiles = ShadowsocksApplication.Companion.app.profileManager.allProfiles
	...
	private fun pingAll()
	{
		...
		PingHelper.instance()
			.pingAll(this, profiles, object : PingCallback()
			{
				...
				{
					mProgressHandler.sendEmptyMessageDelayed(ProfileManagerActivity.Companion.MSG_FULL_TEST_FINISH, 2000)
					PingHelper.instance()
						.releaseTempActivity()
				}
			})
	...
	private inner class ProfileViewHolder(view: View) : RecyclerView.ViewHolder(view), View.OnClickListener, View.OnKeyListener
	{
		...
		fun bind(item: Profile)
		{
			this.item = item
			updateText()
			if (item.id == ShadowsocksApplication.Companion.app.profileId())
			{
				text.isChecked = true
				selectedItem = this
			}
		...
		override fun onClick(v: View)
		{
			ShadowsocksApplication.Companion.app.switchProfile(item.id)
			finish()
		}
	...
	private inner class ProfilesAdapter : RecyclerView.Adapter<ProfileManagerActivity.ProfileViewHolder>()
	{
		...
		init
		{
			list = if (isSort)
			{
				ShadowsocksApplication.Companion.app.profileManager.allProfilesByElapsed.toMutableList()
			}
			else
			{
				ShadowsocksApplication.Companion.app.profileManager.allProfiles.toMutableList()
			}
		...
		fun onGroupChange(groupName: String)
		{
			...
			else
			{
				list = if (isSort)
				{
					ShadowsocksApplication.Companion.app.profileManager.getAllProfilesByGroupOrderByElapse(groupName)
						.toMutableList()
				}
				else
				{
					ShadowsocksApplication.Companion.app.profileManager.getAllProfilesByGroup(groupName)
						.toMutableList()
				}
			}
		...
		override fun onBindViewHolder(vh: ProfileManagerActivity.ProfileViewHolder, i: Int)
		{
			vh.bind(profiles[i])
		}

		override fun onCreateViewHolder(vg: ViewGroup, viewType: Int): ProfileManagerActivity.ProfileViewHolder
		{
		...
		fun move(from: Int, to: Int)
		{
			...
			while (true)
			{
				...
				previousOrder = order
				profiles[i] = next
				ShadowsocksApplication.Companion.app.profileManager.updateProfile(next)
				i += step
			}
			first.userOrder = previousOrder
			profiles[to] = first
			ShadowsocksApplication.Companion.app.profileManager.updateProfile(first)
		...
		fun remove(pos: Int)
		{
			...
			ShadowsocksApplication.Companion.app.profileManager.delProfile(remove.id)
		...
		fun commit(actions: SparseArray<Profile>)
		{
			for (index in 0 until actions.size())
			{
				...
				if (item != null)
				{
					ShadowsocksApplication.Companion.app.profileManager.delProfile(item.id)
					if (item.id == ShadowsocksApplication.Companion.app.profileId())
					{
						ShadowsocksApplication.Companion.app.profileId(-1)
					}
				}
			}
		}
	...
	private inner class SSRSubAdapter : RecyclerView.Adapter<ProfileManagerActivity.SSRSubViewHolder>()
	{
		private var profiles = ShadowsocksApplication.Companion.app.ssrSubManager.allSSRSubs.toMutableList()
		...
		override fun onBindViewHolder(vh: ProfileManagerActivity.SSRSubViewHolder, i: Int)
		{
			vh.bind(profiles[i])
		}

		override fun onCreateViewHolder(vg: ViewGroup, viewType: Int): ProfileManagerActivity.SSRSubViewHolder
		{
			val view = LayoutInflater.from(vg.context)
